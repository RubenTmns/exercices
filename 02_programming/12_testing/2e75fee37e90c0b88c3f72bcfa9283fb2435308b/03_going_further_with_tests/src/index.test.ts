...
describe("Let the tests on arrays begin !", () => {
  ...
  test("If indiana jones is present fail", () => {
    
    mockArray = ["Into the wild", "Alien the 8th passenger","Ip Man","Tombeau des lucioles", "fight club", "Indiana Jones 4"];
    expect(mockArray).not.toContain("Indiana Jones 4");
  })
    
  // Create a test that fails if the list has less than 5 elements.
  test("Fail if less than 5", ()=> {
    
    mockArray = ["Lord of the rings : the return of the king", "Into the wild", "Alien the 8th passenger", "Tombeau des lucioles", "fight club"];
    expect(mockArray.length).not.toBeLessThan(5);
  })
...
test("Checks array contain only strings", () => {
  ...
  test("Passes if there is at least one film Lord of the Rings", () => {
    expect.assertions(1);
    mockArray = ["Lord of the rings : the return of the king", "Into the wild", "Alien the 8th passenger", "Tombeau des lucioles", "fight club"];
